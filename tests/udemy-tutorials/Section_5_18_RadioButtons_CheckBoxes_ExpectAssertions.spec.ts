...
test("Udemy: Implementing Expect Assertions", async ({ page }) => {
  ...
  let isTermsBoxChecked = await page.locator(SELECTORS.termsCheckBox).isChecked();
  ...
  let isUserButtonChecked = await page.locator(SELECTORS.radioButtons).last().isChecked();
  ...
  await page.locator(SELECTORS.dropDownBox).selectOption(`consult`);

  //Select the "User" radio button:
  console.log(`Clicking "User" radio button...`);
  await page.locator(SELECTORS.radioButtons).last().click();
  isUserButtonChecked = await page.locator(SELECTORS.radioButtons).last().isChecked();
  console.log(`User button checked?: ${isUserButtonChecked}`);
  //Assert that "User" button is checked:
  await expect(page.locator(SELECTORS.radioButtons).last()).toBeChecked();

  //Select "Okay" button from the pop-up:
  await page.locator(SELECTORS.popUp_OkayButton).click();

  //Click the terms & conditions check box:
  console.log(`Clicking "Terms" checkbox...`);
  await page.locator(`#terms`).click();
  isTermsBoxChecked = await page.locator(SELECTORS.termsCheckBox).isChecked();
  console.log(`Terms box checked?: ${isTermsBoxChecked}`);
  //Assert that the terms & conditions box is checked:
  await expect(page.locator(SELECTORS.termsCheckBox).last()).toBeChecked();
