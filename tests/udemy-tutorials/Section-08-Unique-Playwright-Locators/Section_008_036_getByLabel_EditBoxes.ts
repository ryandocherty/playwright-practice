...
Another case where it will work is on the angularpractice webpage above.
...
but Playwright will still be able to associate these two elements because there is a reference provided: for="exampleInputPassword1".
...
These values are different, so Playwright will struggle to find an association when just using getByLabel("Password").
