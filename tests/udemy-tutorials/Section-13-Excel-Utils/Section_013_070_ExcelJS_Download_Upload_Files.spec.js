...
Finally we want to validate that the new data appears correctly on the website.
*/
...
async function replaceCellValue(searchText, replacementText, coordModifier, filePath) {
  ...
  //Get the coords of the cell to replace.
  ...
  console.log(
    `Replacing "${cellToReplace.value}" with "${replacementText}" at row ${
      coordinates.row + coordModifier.rowChange
    }, column ${coordinates.column + coordModifier.colChange}`
  );
  ...
  worksheet.eachRow((row, rowNumber) => {
    row.eachCell((cell, colNumber) => {
      ...
      if (cell.value === searchText) {
        //Assign the located cell's coordinates to the "coordinates" object properties:
